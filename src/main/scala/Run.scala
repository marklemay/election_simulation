import VoteSystem.RankChoice.OptimalGameTheory
import VoteSystem.VoteSystem

import java.io.{File, FileWriter}
import scala.util.Random


def runsystem(election: VoteSystem, voters: Seq[Seq[Double]]): Unit = {
...
import election.Ballot
    ...
    for (voter <- Range(0, voters.size)) {
      ...
      for (_ <- Range(0, samplesPerStep)) {
        ...
        for (myBallot <- election.allBallots) {
          ...
          winners.dist.map((c,p) => util(voter)(c)*p).sum

          val EV = winners.dist.map((c,p) => util(voter)(c)*p).sum
          EVs(myBallot) += EV
        }
      }
  ...
  var publicProbs = Seq.fill(voters.size)(Sampler(election.allBallots.map(b => (b, 1.0 / election.allBallots.size)).toMap))


  var aggregatePreference = election.candidates.map(i => voters.map(voter => voter(i)).sum)

  println(aggregatePreference)
  var (prefBest, best) = aggregatePreference.zipWithIndex.maxBy(_._1)


  val (honestVote, publicProbs1) = NextProbBySmaple(publicProbs, voters, .5)
  ...
  for (i <- Range(2, 7)) {
    var (finalVote1, publicProbs1) = NextProbBySmaple(publicProbs, voters, (1.0 - 1.0 / i.toDouble))
...
//  }).sum.toDouble / voters.size.toDouble
  ...
  // exelerate
  for (i <- Range(7, 20)) {
    var (finalVote1, publicProbs1) = NextProbBySmaple(publicProbs, voters, (1.0 - 1.0 / 7.0))
...
//
//
//  val diffBallots = honestVote.zip(finalVote).map((honest, strategic) => if (honest == strategic) {
//    0
//  } else {
//    1
//  }).sum.toDouble / voters.size.toDouble
